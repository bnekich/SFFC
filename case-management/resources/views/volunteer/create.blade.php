...
@section('content')
    ...
    <form class="space-y-6" action="{{ route('volunteer.store') }}" method="POST">
        @csrf
        <div>
            <label for="sffc_chapter_id" class="sffc-label">Safe Families Nearest You<span
                    class=" text-red-500">*</span></label>
            <select id="sffc_chapter_id" name="organization_id"
                class="sffc-text-input @error('gender') border-red-500 @enderror" required>
                <option value=""> (Select)</option>
                @foreach ($sffc_locations as $sffc_location)
                    <option value="{{ $sffc_location->id }}"
                        {{ old('sffc_chapter_id') == $sffc_location->id ? 'selected' : '' }}>
                        {{ $sffc_location->name }}
                    </option>
                @endforeach
            </select>
            @error('sffc_chapter_id')
