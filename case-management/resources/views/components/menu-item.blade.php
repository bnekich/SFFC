...
@if (empty($link['permission']) || auth()->user()->can($link['permission']))
    ...
    <li x-data="{ open: false }" class="relative">
        @if (isset($link['action']) && $link['action'] === 'logout')
        ...
        @elseif(isset($link['submenu']))
            <button @click="open = !open"
                ...
                {{ $link['name'] }}
                <svg class="w-4 h-4 ml-2 transition-transform" :class="{ 'transform rotate-180': open }"
            ...
            <ul x-show="open" x-transition:enter="transition ease-out duration-200"
                ...
                @foreach ($link['submenu'] as $subitem)
                    <x-menu-item :link="$subitem" />
                @endforeach
            </ul>
        @else
            <a href="{{ $link['url'] }}" class="block py-2 px-4 rounded hover:bg-gray-700">
                {{ $link['name'] }}
            </a>
        @endif
    </li>
